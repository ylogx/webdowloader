...
try:
    ...
    print('Type: `wget https://raw.githubusercontent.com/shubhamchaudhary/python/master/file_downloader.py`')
    sys.exit(1)
...
def parse(url,extension=None,destination=None):
    ...
    for address in href_list:
        if (not address): continue      #If address in NoneType
        if (address[-len(extension):] == extension or address[-4:] == 'jpeg' or address[-3:] == 'png'):
...
def parse_filetype(url,filetype,destination):
    ...
    if filetype == 'i' or filetype == 'image' or filetype == 'images' or filetype == 'img':
        EXTENSIONS = IMAGE_EXTENSIONS
    elif filetype == 'd' or filetype == 'doc' or filetype == 'pdf':
        EXTENSIONS = DOCUMENT_EXTENSIONS
    elif filetype == 'v' or filetype == 'video' or filetype == 'videos' or filetype == 'movie' or filetype == 'movies':
        EXTENSIONS = VIDEO_EXTENSIONS
    elif filetype == 'a' or filetype == 'audio' or filetype == 'sound':
...
def main(argv):
    ...
    parser.add_option("-e", "--extension", dest="extension",
                     help="Download files with this extension .EXT", metavar="EXT")
    parser.add_option("-d", "--destination", dest="destination",
                     help="destination FOLDER where files will be saved", metavar="FOLDER")
    parser.add_option("-f", "--file", dest="filename",
                     help="parse a downloaded html FILE", metavar="FILE")
    parser.add_option("-t", "--type", dest="filetype",
                     help="Download files of this filetype: e.g image, videos, doc")
...
if __name__ == '__main__':
    try:
        ...
        print('\nGracefully exiting, but that is not fair. I was trying to get it for you :(')
